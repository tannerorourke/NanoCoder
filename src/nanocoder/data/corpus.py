...
def apply_token_fim(ids, fim_prefix_id, fim_middle_id, fim_suffix_id, spm_rate=0.5, rng=RNG):
    ...
    Splits the *already tokenized* id stream at two token boundaries and reorders it, so 
    the cut never lands mid-token and the sentinels are real single tokens. With prob 
    spm_rate we emit SPM form (suffix first), else PSM:
...
def compile_corpus(tokenizer, texts, fim_rate, spm_rate=0.5, add_eos=True, rng=RNG):
    """
    array('H') is a flat C buffer of uint16: ~2 bytes/token. A Python list of ids
    ...
    for t in tqdm(texts, desc="Compiling"):
        ids = tokenizer.encode(t)                       # indent/dedent applied; NO fim here
